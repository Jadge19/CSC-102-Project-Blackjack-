...
public class Blackjack
{
	public static String options(Hand playerHand, int funds, int score)  //provides options when split or doubling down is possible
	{
		...
		while (!(ans.equals("s") || ans.equals("h") || ans.equals("w") || ans.equals("d"))) //user must enter one of these
		{
			...
			/*
				can perform a split:
			*/
			else if (((funds*2) <= score) && ((playerHand.getHandSize() == 2) && (playerHand.getHandTotal() == 20) && (playerHand.doubleTen())) || ((playerHand.getHandSize() == 2) && (playerHand.doubleAce()))) //can split
			{
	...
	public static void main(String[] args)
	{
		...
		while (score > 4)                //can keep playing 
		{
			...
			while ((bet < 5) || (bet > score)); //|| (poor == true));  //bet must be higher than 5 and less than or equal to score(funds)
			System.out.println();
